...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(filter_var($vEmail, FILTER_VALIDATE_EMAIL)) {
        ...
        //TODO Prepojit s tabulkou TD-Timy
      //TODO Prepisat z "->" do "mysqli_....."
      //TODO Prelozit + Vycistit
      if (empty($vEmail_err) && empty($vPass_err)) {
        $sql = "SELECT `tName`, `tHeslo` FROM `TD-Timy` WHERE (`tName` = ? AND `tHeslo` = ?) ";

        if ($stmt = $conn->prepare($sql)) {
          $stmt->bind_param("ss", $param_username, $param_password);
          $param_username = $vEmail;
          $param_password = $vPass;
